...
| getmatch.ru | combined | listing `/vacancies` | No free-text search box; `?query=` is stripped. Target roles stay on `_search_keywords` and are matched against rich `/api/offers` cards while offset pagination follows `meta.total`; canonical detail pages are fetched concurrently, with API-card fallback for temporary detail loss and sitemap/browser-scroll fallback for listing loss. Shared retry propagates 429/503 to durable ingest. |
