...
_TARGETS = [
    ("kadrof", "https://www.kadrof.ru/work", ["https://www.kadrof.ru/work/remote-work", "https://www.kadrof.ru/work/moscow", "https://www.kadrof.ru/work/all"],
     "https://www.kadrof.ru/work"),
    ("telecom_kz", "https://telecom.kz/ru/career", ["https://telecom.kz/ru/news", "https://telecom.kz/ru/company", "https://telecom.kz/ru/career"],
     "https://telecom.kz/ru/career"),
    ("airastana", "https://airastana.com/kaz/en-us/About-Us/Careers", ["https://airastana.com/kaz/en-us/About-Us/Careers"],
...
def _save(out: Path, name: str, data: dict) -> None:
    (out / f"{name}.json").write_text(json.dumps(data, ensure_ascii=False, indent=1), encoding="utf-8-sig")


async def _full_evidence(service: OperatorBrowserSessionService, sid: str, out: Path, tag: str) -> dict:
    ...
    return {"challenge": snap.get("challenge"), "final_url": snap.get("final_url"), "page_title": snap.get("page_title"), "saved": saved, "trace_kind": trace.get("trace", {}).get("kind")}


async def _churn(service: OperatorBrowserSessionService, name: str, url: str, paths: list[str], out: Path) -> dict:
    site_out = out / name
    ...
    for i in range(_MAX_NAV):
        ...
        navs.append({"url": nav.get("final_url", paths[i % len(paths)]), "status": nav.get("status"), "challenge": ch})
        if ch:
            ...
            solved = {"result": solve.get("captcha"), "challenge_after": post.get("challenge"), "cleared": not post.get("challenge")}
            result["solve"] = solved
            _save(site_out, "solve_result", solved)
            nav = await service.continue_session(sid, f"navigate {paths[i % len(paths)]}")
            navs.append({"retry_after_solve_status": nav.get("status"), "challenge": nav.get("challenge")})
