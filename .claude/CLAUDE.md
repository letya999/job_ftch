...
- `application/`: Protocol ports, extension registry, pipeline orchestration, rejection conversion, logging, telemetry.
- `infrastructure/`: adapters for fixture files, Telegram, career-site HTTP/HTML, in-memory store, and PostgreSQL store.
- `nodes/`: processing nodes. Current runtime chain starts with `SanitizeNode` and may continue through `HeuristicTriageNode` and `DedupNode`.
- `sinks/`: output adapters. Currently only `JsonFileSink`.
- `app.py`: composition root and CLI entry point.
- `config.py`: `JOB_FTCH_` settings, backend keys, and runtime validation.
...
- `Pipeline.run` tracks fetched, sanitized, triaged, dropped, emitted, quarantined, failed, and per-source counters.
...
- `SanitizeNode` must remain the mandatory first processing step. `HeuristicTriageNode` and `DedupNode` are optional processing stages after sanitation.
