    ...
    for kind, sources in grouped.items():
        ...
        for s in sources:
            ...
            if (
                ...
                and kind in ("telegram_channel", "telegram_group")
            ):
...
async def callback_run_from_sources(
    callback: CallbackQuery, runner: "TenantRunner", bot: Bot
) -> None:
...
async def callback_clear_sources(callback: CallbackQuery, runner: "TenantRunner") -> None:
    tenant_id = runner.default_tenant_id()
