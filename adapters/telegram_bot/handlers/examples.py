...
async def _get_tenant_and_profile(runner: "TenantRunner", from_user_id: int | None) -> tuple[str, str, str]:
...
async def cmd_positive(message: Message, state: FSMContext, runner: "TenantRunner", user_id_override: int | None = None) -> None:
...
async def cmd_negative(message: Message, state: FSMContext, runner: "TenantRunner", user_id_override: int | None = None) -> None:
...
async def cmd_done(message: Message, state: FSMContext, runner: "TenantRunner") -> None:
    tenant_id, user_id, profile_id = await _get_tenant_and_profile(runner, message.from_user.id if message.from_user else None)
    profile = await runner.get_candidate_profile(tenant_id, user_id, profile_id)
    pos, neg = _get_example_counts(profile)
    await state.clear()
    await message.answer(
...
async def handle_document_example(message: Message, state: FSMContext, runner: "TenantRunner", bot: Bot) -> None:
    ...
    if is_limit:
        ...
        await status_msg.edit_text(f"✅ #{count} добавлен. Ролей: {roles_preview}. Ещё {10-count} слотов.")
...
async def cmd_examples(message: Message, runner: "TenantRunner", user_id_override: int | None = None) -> None:
    resolved_uid = user_id_override if user_id_override is not None else (message.from_user.id if message.from_user else None)
    tenant_id, user_id, profile_id = await _get_tenant_and_profile(runner, resolved_uid)
    profile = await runner.get_candidate_profile(tenant_id, user_id, profile_id)
    pos, neg = _get_example_counts(profile)
...
async def _show_example_page(callback: CallbackQuery, examples: tuple[str, ...], idx: int, kind: Literal["pos", "neg"]) -> None:
    ...
    builder.button(text="🗑 Удалить этот", callback_data=ExampleNav(action=f"del_one_{kind}", idx=idx))
...
async def callback_show_examples(callback: CallbackQuery, callback_data: ExampleNav, runner: "TenantRunner") -> None:
...
@router.callback_query(ExampleNav.filter(F.action.in_(["prev_pos", "next_pos", "prev_neg", "next_neg"])))
async def callback_nav_examples(callback: CallbackQuery, callback_data: ExampleNav, runner: "TenantRunner") -> None:
...
async def callback_del_one(callback: CallbackQuery, callback_data: ExampleNav, runner: "TenantRunner") -> None:
    kind: Literal["pos", "neg"] = "pos" if "pos" in callback_data.action else "neg"
...
async def callback_del_all(callback: CallbackQuery, runner: "TenantRunner") -> None:
    ...
    msg = callback.message
    if isinstance(msg, Message):
        await msg.edit_text(f"⚠️ Удалить ВСЕ примеры ({pos}+ и {neg}−)?", reply_markup=builder.as_markup())
