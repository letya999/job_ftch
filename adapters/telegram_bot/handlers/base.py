...
async def cmd_start(message: Message, runner: "TenantRunner") -> None:
    ...
    builder.button(text=f"📋 Примеры ({pos}/{neg})", callback_data=StartMenu(action="show_examples"))
    builder.button(text=f"🔗 Источники ({k_sources})", callback_data=StartMenu(action="show_sources"))
