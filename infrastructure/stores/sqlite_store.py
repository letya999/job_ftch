...
from domain import Job, RawItem
...
class SQLiteStore:
    ...
    def _init_tables(self) -> None:
        ...
        with self._get_connection() as conn:
            ...
            # Индексы
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_jobs_source ON jobs(source_kind, source_name)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_raw_source ON raw_items(source_kind, source_name)"
            )
    ...
    # Аннотации типов для sqlite_store
    ...
    async def is_processed(self, item_id: str) -> bool:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute("SELECT processed FROM raw_items WHERE stable_id = ?", (item_id,))
            row = cursor.fetchone()
    ...
    async def get_last_state(self, key: str) -> str | None:
    ...
    async def save_job(self, job: Job) -> str:
        ...
        with self._get_connection() as conn:
            compensation = job.compensation  # ИСПРАВЛЕНО: было compensatio
            conn.execute(
                """
    ...
    # Дополнительные методы для фильтрации по типу источника
    async def get_external_jobs(self, limit: int = 500) -> list[dict[str, Any]]:
        """Get only external jobs (CAREER_SITE source)."""
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
                SELECT * FROM jobs 
                WHERE source_kind = 'career_site'
    ...
    async def get_telegram_jobs(self, limit: int = 500) -> list[dict[str, Any]]:
        """Get only Telegram jobs."""
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
                SELECT * FROM jobs 
                WHERE source_kind IN ('telegram_channel', 'telegram_group', 'telegram_comment')
                ORDER BY found_at DESC
                LIMIT ?
            """,
                (limit,),
