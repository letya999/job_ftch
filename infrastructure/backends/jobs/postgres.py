...
class PostgreSQLJobBackend(JobPersistenceBackend, JobGroupStore, SearchBackend):
            ...
            async with conn.transaction():
                ...
                if group_id:
                    row = await conn.fetchrow(
                        "SELECT raw_json FROM jf_job_groups WHERE group_id = $1",
                        group_id
                    )
                    if row:
    ...
    async def list_jobs(self, limit: int, offset: int) -> list[Job]:
        pool = await self._get_pool()
        async with pool.acquire() as conn:
            rows = await conn.fetch("SELECT raw_json FROM jf_jobs ORDER BY updated_at DESC LIMIT $1 OFFSET $2", limit, offset)
    ...
    async def search(self, query: str, limit: int = 20) -> list[JobGroup]:
        ...
        async with pool.acquire() as conn:
            ...
            grows = await conn.fetch("SELECT group_id, raw_json FROM jf_job_groups WHERE group_id = ANY($1)", group_ids)
