...
from job_ftch.infrastructure.sources.site_fingerprinter import fingerprint, SiteClass, SiteProfile
...
async def test_fingerprint_ssr_via_vacancy_links():
    ...
    with patch("job_ftch.infrastructure.sources.site_fingerprinter.httpx.AsyncClient", return_value=mock_client):
...
async def test_get_ordered_monitors_fallback_on_error():
    with patch("job_ftch.infrastructure.sources.monitor_detector.fingerprint", side_effect=Exception("Fingerprint error")):
        monitors = await get_ordered_monitors("https://example.com/jobs", None)
