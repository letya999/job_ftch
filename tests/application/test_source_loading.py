...
def test_career_site_config_from_spec_auto_greenhouse_no_url_substring():
    """ADR-033: declarative config no longer dispatches on URL substring.

    With parser_kind="auto" and a greenhouse.io URL, the result is now
    "generic" because no registered site_parser declares parser_kind="greenhouse"
    for that domain. The new substring-match auto-detect was removed to comply
    with the no-host-hardcode rule. Callers who want greenhouse behaviour must
    set parser_kind="greenhouse" explicitly, or the URL must match a registered
    site parser that returns parser_kind="greenhouse".
    """
    ...
    spec = DeclarativeHtmlSpec(url="https://boards.greenhouse.io/myco", parser_kind="auto")
