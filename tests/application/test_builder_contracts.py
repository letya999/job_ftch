...
from job_ftch.application.builder import build_delivery_targets, build_output_sinks
...
async def test_openai_provider_forwards_timeout_retry_and_prompt(
    ...
    class FakeClient:
        async def create_with_completion(
            ...
            return {"ok": True}, SimpleNamespace(usage=None)
...
async def test_build_output_sinks_routes_borderline_jobs_to_review(
    ...
    sink, main_sink, review_sink, posting_sink = build_output_sinks(settings)
    borderline = _record(
        quality_score=0.4,
        review_reasons=("partial_extraction",),
        routing_decision=MatchDecision.REVIEW,
    )
    strong = _record(quality_score=0.9, review_reasons=(), routing_decision=MatchDecision.ACCEPT)
...
async def test_build_output_sinks_respects_dry_run_for_posting(
    ...
    _, _, _, posting_sink = build_output_sinks(settings)
...
async def test_build_output_sinks_routes_strong_jobs_to_posting(
    ...
    targets = build_delivery_targets(settings, posting_sink)
    assert len(targets) == 1
    await targets[0].deliver(candidate)
    await sink.flush()
...
@pytest.mark.asyncio
async def test_build_output_sinks_does_not_emit_hard_rejects(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    class RecordingSink:
    ...
    rejected = _record(
        quality_score=0.4,
        review_reasons=("partial_extraction", "llm_relevance:reject"),
        routing_decision=MatchDecision.REJECT,
    )

    await sink.emit(rejected)
    await sink.flush()

    assert posting_sink is None
    assert main_sink.emit_count == 0
    assert review_sink.emit_count == 0
    assert len(sinks) == 2
    assert sinks[0].items == []
    assert sinks[1].items == []
