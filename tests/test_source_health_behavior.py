...
from job_ftch.domain import SourceHealth, SourceKind, TenantConfig, source_spec_identifier
...
async def test_source_health_thresholds_from_settings() -> None:
    ...
    previous = SourceHealth(
        source_id=sid, source_kind="test", source_name="src",
        last_run_at=datetime.now(UTC).isoformat(),
        last_success_at=None,
        failure_streak=4,
        success_count=0,
        last_fetched=0, last_emitted=0, last_failed=1, last_quarantined=0,
        baseline_emitted=0.0, drift_ratio=None, degraded=False,
        status="healthy", paused=False
    )
