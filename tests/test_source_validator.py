...
async def test_check_url_reachable_exception():
    with patch("httpx.AsyncClient.head", side_effect=Exception("network error")):
...
async def test_validate_sources_mixed():
    ...
    with patch("job_ftch.adapters.source_validator.check_url_reachable", side_effect=mock_reachable):
