...
def test_default_profile_matches_hardcoded_values():
    profile = FilterProfile.default()
    assert "llm" in profile.positive_relevance_keywords
    assert "ai" in profile.positive_relevance_keywords
    assert "genai" in profile.positive_relevance_keywords
    assert "sales" in profile.negative_relevance_keywords
    assert "recruiter" in profile.negative_relevance_keywords
...
@pytest.mark.asyncio
async def test_triage_node_default_profile_backward_compatible():
    node = HeuristicTriageNode()  # default profile
    # Should drop webinar
...
@pytest.mark.asyncio
async def test_relevance_node_default_profile_backward_compatible():
