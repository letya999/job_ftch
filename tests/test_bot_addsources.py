...
async def test_bot_addsources_command():
    ...
    runner.add_source_spec = AsyncMock()
    ...
    with MagicMock():
        ...
        bot_module.build_source_spec_from_input = AsyncMock(
            return_value=TelegramChannelSpec(entity="test_chan")
        )

        try:
            await service.handle_command(
                "/addsources test_tenant @test_chan @another_chan", chat_id=456, user_id=123
            )

            assert runner.add_source_spec.call_count == 2
            sender.send_message.assert_called_with(456, "All 2 sources added.")
        finally:
