...
добавляет YAML-конфигурируемый рендерер карточек Telegram с дрип-пейсингом и
дедупликацией, усиливает извлечение с career-сайтов и парсер hirify, делает
per-run LLM-бюджет атомарной резервацией, исправляет авто-паузу здоровья
источников и пинует actions основного CI-workflow на коммит-SHA.

### Добавлено

- **YAML-конфигурируемый публикационный слой**: рендерер карточек Telegram из
  конфига (`config/publication/card.yaml`), дрип-пейсинг, чередование и
  дедупликация по фингерпринту, плюс локализованные подписи карточки с доменом
  источника в футере и eval-скрипты с golden-snapshot тестами.

### Изменено

- **Извлечение предпочитает структурные метаданные сайта** выводу из прозы;
  поля для читателя (заголовок, требования, стек) остаются на языке источника, а
  свободный текст локации спасается только когда это не настоящее место.
- **Per-run LLM-лимит** теперь енфорсится атомарным `AsyncCallBudget`, общим для
  LLM-ноды, с размером `min(pipeline_max_llm_calls_per_run,
  llm_relevance_max_per_run)`, вместо счётчика воркера постфактум (#150).
- **Гигиена репозитория**: скрипты и политика безопасности перенесены под
  `.repo-safety/` (каноничная раскладка ai-repo-safety):
  `scripts/security/*.py` → `.repo-safety/scripts/`, `.repo-safety.json` →
  `.repo-safety/config.json`; ссылки в pre-commit и pre-push обновлены. Удалены
  избыточные `.cursorrules`, `GEMINI.md` и файлы-заготовки `*.ai-repo-safety.*`.
- **Версии CI actions**: `codecov/codecov-action` v7, `ossf/scorecard-action`
  2.4.4 и `github/codeql-action/upload-sarif` v4.37.3 (вобрало dependabot #155,
  #156, #157).

### Исправлено

- **Авто-пауза здоровья источника** (#147): доля отказов теперь `failed/fetched`,
  а не `failed/(fetched+failed)` — прежняя никогда не превышала 0.5, поэтому
  частично сломанные источники сбрасывали серию отказов и обходили настроенную
  авто-паузу.
- **Парсер hirify**: тело вакансии берётся из first-party API, краул оставлен
  фолбэком, а `tools_stack` фолбэчит на теги из API.
- **Метки источника Telegram**: приватный invite-токен никогда не показывается
  как публичная метка источника.
- **auth_telethon**: пароль 2FA читается через `getpass`, а один dotenv-файл
  выбирается флагом `--env-file`.
- **Кэш presentable**: ключи кэша версионируются и инвалидируются при смене
  промпта или схемы.

### Безопасность

- **Actions CI запинены на коммит-SHA** (#153): `ci.yml` больше не исполняет
  подвижные мажорные теги, что закрывает путь дрейфа/компрометации цепочки
  поставок на самом частом workflow, чей шаг покрытия также работает с
  `CODECOV_TOKEN`. Per-run лимит браузерных навигаций, который item-воркер на
  деле не енфорсил, теперь пишет предупреждение на этапе сборки вместо имитации
  жёсткого потолка; реальный енфорсмент вынесен в техдолг.
