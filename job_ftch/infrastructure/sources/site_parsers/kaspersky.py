"""Discover-only parser for careers.kaspersky.ru."""
...
class KasperskyCareerParser:
    domain_pattern = r"^https?://careers\.kaspersky\.ru(?:/|$)"
    ...
    def _detail_re(self) -> re.Pattern[str]:
        ...
        pattern = getattr(manifest_entry, "detail_pattern", None) or r"/vacancy/\d+(?:/|$)"
    ...
    def __name__(self) -> str:
        return "KasperskyCareerParser"


register_site_parser(
    "kaspersky_career",
    domain_pattern=KasperskyCareerParser.domain_pattern,
    assessment_hint=known_board_assessment_hint(
        "known_site",
        "site_parser:careers.kaspersky.ru",
        has_stable_url=True,
        supports_ordered_head=False,
        can_detect_freshness_without_snapshot=False,
        ordered_by_newest=False,
        requires_full_snapshot=False,
        rationale="careers.kaspersky.ru renders canonical /vacancy/<id> links in SSR HTML, so a lightweight dedicated parser can avoid the generic detail extraction misses.",
    ),
