...
PINIA_STATE_RE = re.compile(r'window\.__PINIA_STATE__\s*=\s*"(.+?)"\s*;?\s*</script>', re.DOTALL)
...
def extract_field(
    ...
    if result is None:
        return None
    # Return complex types as-is; only stringify scalars
...
def extract_pinia_state(html: str) -> dict[str, Any] | None:
    """Extract data from window.__PINIA_STATE__ (Vue/Pinia frameworks).

    The value is typically a JSON string with escaped quotes (\\\" -> \").
    We unescape and parse it, then look for nested job arrays.
    """
    match = PINIA_STATE_RE.search(html)
    if not match:
        return None
    raw_str = match.group(1)
    try:
        unescaped = raw_str.encode().decode("unicode_escape")
